...
const COLS_REMISE = {
  ...
  suivi_ref_start: 90, // CM — référence Suivi par chèque (1 colonne par chèque, jusqu'à 10), après les 10 blocs de détail (10..89)
};
...
async function saveRemise(remise) {
  ...
  const mainRow = new Array(Math.max(10 + remise.cheques.length * 8, COLS_REMISE.suivi_ref_start + remise.cheques.length)).fill('');
...
// Calcule le solde de clôture d'un compte pour une période donnée
// (solde initial de la période + somme des mouvements de la période).
// Sert à reporter automatiquement le solde vers la période suivante.
async function getSoldeFinal(periode, compte) {
  const soldeInit = getSoldeInitial(periode, compte);
  let rows = [];
  if (compte === 'banque') {
    rows = await getBanqueOperations(periode);
    const mvt = rows.reduce((s, r) =>
      s + (parseFloat(r[COLS_BANQUE.credit]) || 0) - (parseFloat(r[COLS_BANQUE.debit]) || 0), 0);
    return Math.round((soldeInit + mvt) * 100) / 100;
  }
  if (compte === 'caisse1') rows = await getCaisseOperations(periode);
  else if (compte === 'caisse2') rows = await getCaisse2Operations(periode);
  else return soldeInit;
  const mvt = rows.reduce((s, r) =>
    s + (parseFloat(r[COLS_CAISSE.credit]) || 0) - (parseFloat(r[COLS_CAISSE.debit]) || 0), 0);
  return Math.round((soldeInit + mvt) * 100) / 100;
}
...
// Retourne la période précédant celle donnée (ex: "2024 - 2025" -> "2023 - 2024"),
// ou null si c'est la première période connue.
function getPreviousPeriode(periode) {
  const periodes = getPeriodes();
  const idx = periodes.indexOf(periode);
  return idx > 0 ? periodes[idx - 1] : null;
}
...
window.Sheets = {
  ...
  // Soldes
  getSoldeInitial, saveSoldeInitial, saveSoldesInitiauxBatch, getSoldeFinal,
  // Calculs
